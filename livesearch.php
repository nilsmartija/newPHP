...
try {
    ...
    $query = $connection->prepare("SELECT users.UserID, users.firstname, users.lastname, users.birthday, users.sex, users.Username, users.user_profile_picture, CONCAT(user_add_city, ', ', user_add_province) AS address FROM users INNER JOIN user_address ON users.UserID = user_address.UserID LIMIT :offset, :recordsPerPage");
